...
public function ajouterMenu($Menu,$folder){
    $sql="insert into Menu(nom_plat,prix_plat,image_plat) values(:nom_plat,:prix_plat,:folder)";
    $db=config::getConnexion();
    try{
    $req=$db->prepare($sql);
    ...
    public function findproduit($id){
        try{
            $db=config::getConnexion();
            $query=$db->prepare('SELECT * FROM menu WHERE id = :id');//reqt
